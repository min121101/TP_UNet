...
def load_LITS_img(path, CFG):
    if not CFG['2.5D']:
        img = cv2.imread(path)
        if img is None:
            raise FileNotFoundError(f'Failed to read LITS image (path missing or unreadable): {path}')
        # img = np.tile(img[..., None], [1, 1, 3])
    else:
        img = np.load(path)
    img = img.astype('float32') # original is uint16
...
def load_LITS_msk(path):
    msk = cv2.imread(path, cv2.IMREAD_UNCHANGED)
    if msk is None:
        raise FileNotFoundError(f'Failed to read LITS mask: {path}')
    msk = msk.astype('float32')
    msk /= 255.0
    return msk

def load_LITS_msks(path):
    msk = np.load(path)
    msk = msk.astype('float32')
    msk /= 255.0
    return msk
