...
class RGA_ResNetEncoder(ResNetEncoder):
        def get_stages(self):
            return [
            nn.Identity(),
            nn.Sequential(self.conv1, self.bn1, self.relu),
            nn.Sequential(self.maxpool, self.layer1, self.rga_att1),
            nn.Sequential(self.layer2, self.rga_att2),
            nn.Sequential(self.layer3, self.rga_att3),
            nn.Sequential(self.layer4, self.rga_att4),
        ]
