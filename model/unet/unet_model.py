...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False):
        ...
        self.Skip_Att = nn.Sequential(OrderedDict([
            ('unit1', Skip_CrossAtt(vis_dim=128, lang_dim=512, head_num=1, emb=128)),
            ('unit2', Skip_CrossAtt(vis_dim=1024, lang_dim=512, head_num=1, emb=1024)),
        ]))
        self.text_linear = torch.nn.Linear(3328, 512) # LITS
        self.temporal_linear = torch.nn.Linear(1280, 512)  # LITS
        # self.linear = torch.nn.Linear(512, 512)

        self.low_level_loss_conv1x1 = nn.Conv2d(in_channels=128, out_channels=1, kernel_size=1, stride=1, padding=0)
        self.low_level_loss_linear = nn.Linear(9216, 512)
        self.Relu = nn.ReLU()

    def forward(self, x, text, temporal):
        text_feature = text.flatten(1).unsqueeze(1)
        text_feature = self.text_linear(text_feature)
        temporal = temporal.flatten(1).unsqueeze(1)
        temporal = self.temporal_linear(temporal)
        text_feature = (text_feature + temporal)
        x1 = self.inc(x)
        x2 = self.down1(x1)
