...
sys.path.insert(0, '.')
from CrossUnet.common import *

sys.path.insert(0, '../utils/')
from CrossUnet.utils.helpers import *
...
#         self.cas = nn.ModuleList([Skip_CrossAtt(vis_dim=2048, lang_dim=1000)])
#
...
class Skip_CrossAtt(nn.Module):
    ...
    def forward(self, vis, emb):  # vis: (B,C,H,W), emb: (B, L, C')
        ...
        vis_att = self.convSA(vis)  # [1, 3, 10, 10]

        lang_att = self.linearSA(emb).transpose(1, 2)  # [1, 3, 10]


        vis_ = vis_att.unsqueeze(2).repeat(1, 1, L, 1, 1) # [1, 3, 10, 10, 10]
        emb_ = lang_att.unsqueeze(3).unsqueeze(3).repeat(1, 1, 1, H, W) # [1, 3, 10, 10, 10]
        multi = torch.cat((vis_, emb_), 1)  # [1, 6, 10, 10, 10]
        multi = multi.transpose(1, 2).reshape(B * L, -1, H, W) #[10, 6, 10, 10]

        query = self.Query(multi).view(B, L, -1, H, W) # [1, 10, 3, 10, 10]
        key = self.Key(multi).view(B, L, -1, H, W) # [1, 10, 3, 10, 10]
        value = self.Value(multi).view(B, L, -1, H, W) # [1, 10, 3, 10, 10]
        res_value = self.resValue(multi).view(B, L, -1, H, W) # [1, 10, 3, 10, 10]

        query = query.transpose(1, 2) # [1, 3, 10, 10, 10]
        key = key.transpose(1, 2) # [1, 3, 10, 10, 10]
        value = value.transpose(1, 2) # [1, 3, 10, 10, 10]
        res_value = res_value.transpose(1, 2) # [1, 3, 10, 10, 10]
        ...
        return value_att + vis

class Down_CrossAtt(nn.Module):
    def __init__(self, vis_dim=2048, lang_dim=1000, head_num=8, emb=512):
        super(Down_CrossAtt, self).__init__()
    ...
    def forward(self, vis, emb):  # vis: (B,C,H,W), emb: (B, L, C')
        ...
        v_att = torch.bmm(value_, att.transpose(1, 2))
        v_att = v_att.reshape(B, -1, L, H, W)
        value_att = torch.mean(v_att + res_value, (3,4)).transpose(1,2)

        return value_att + emb


if __name__ == '__main__':
    print('=='*25)
    print('Image Process')
    img_features = torch.rand((1, 3, 10, 10))
    B, C, H, W = img_features.size()
    text_features = torch.rand((1, 12, 20))
    skip_att = Skip_CrossAtt(vis_dim=3, lang_dim=20, head_num=1, emb=3)
    value = skip_att(img_features, text_features)
    print(value.shape)
    print('==' * 25)
    print('=='*25)
    print('Text Process')
    ...
    B, C, H, W = img_features.size()
    text_features = torch.rand((1, 12, 20))
    down_att = Down_CrossAtt(vis_dim=3, lang_dim=20, head_num=1, emb=20)
    value = down_att(img_features, text_features)
    print(value.shape)
    print('==' * 25)